...
Options:
  ...
  if (!semver.satisfies(process.versions.node, packageInfo.engines.node)) {
    console.error(`${packageInfo.name}: Node.js ${process.versions.node} is unsupported. `
      + `Upgrade Node.js to a version matching ${packageInfo.engines.node} before starting the server.`);
    process.exitCode = 1;
    return;
  }
