...
**Tech Stack:** TypeScript, Node.js, `@xterm/headless`, `@modelcontextprotocol/sdk`, `node-pty`, Zod, and Node's built-in test runner.
...
- Runtime: Node.js 22 or newer; TypeScript with strict checking and ESM output.
- Runtime dependencies: `@xterm/headless` 6.0.0, `@modelcontextprotocol/sdk` 1.30.1, `node-pty` 1.2.0-beta.15, and Zod 4.
...
- [x] **Implement the executable and shutdown.** Read package identity/version from `package.json` through one small shared metadata module. Handle `--help`/`-h`, `--version`/`-v`, and invalid arguments before dynamically loading server/native dependencies; test these paths from an isolated package without dependencies. Add `#!/usr/bin/env node` at the top of `src/index.ts`, mark the compiled entry point executable in the build command, connect `StdioServerTransport`, and route diagnostics to stderr. A single idempotent shutdown promise stops protocol admission before awaiting every registry cleanup on stdin EOF, stdout stream failure, SIGHUP, SIGINT, or SIGTERM. Consume writable stream errors so EPIPE cannot terminate the process before cleanup. Do not call immediate `process.exit` before cleanup completes. Never write child output directly to server stdout.
