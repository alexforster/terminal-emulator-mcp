...
- [ ] **Implement the executable and shutdown.** Read package identity/version from `package.json` through one small shared metadata module. Handle `--help`/`-h`, `--version`/`-v`, and invalid arguments before dynamically loading server/native dependencies; test these paths from an isolated package without dependencies. Add `#!/usr/bin/env node` at the top of `src/index.ts`, connect `StdioServerTransport`, and route diagnostics to stderr. A single idempotent shutdown promise closes the registry and transport on disconnect or termination signals. Do not call immediate `process.exit` before cleanup completes. Never write child output directly to server stdout.
...
CI runs `npm ci`, `npm run typecheck`, `npm test`, and `npm pack --dry-run` on `ubuntu-latest` and `macos-latest`, with Node 22 and 24. Use current maintained checkout/setup-node actions verified at implementation time; do not invent action versions.
