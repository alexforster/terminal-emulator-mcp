...
test("release changes update matching GitHub tarball URLs without rewriting other versions or packages", () => {
  const base = "https://github.com/example/terminal-emulator-mcp/releases/download/";
  const archive = (version) => `${base}v${version}/terminal-emulator-mcp-${version}.tgz`;
  const other = `${base}v0.9.0/another-package-0.9.0.tgz`;
  const readme = [archive("0.9.0"), archive("0.9.0-rc.1"), archive("0.9.00"), other].join("\n");
  assert.equal(
    nextRelease(packageFiles("1.2.3", "0.9.0", readme)).files["README.md"],
    [archive("0.9.1"), archive("0.9.0-rc.1"), archive("0.9.00"), other].join("\n"),
  );
});
...
test("an unreachable release PR cannot block a reused version or subsequent dependency releases", async () => {
  const f = fixture();
  f.commit("obsolete-release", packageFiles("1.2.3", "0.9.1"), "base");
  f.commit("published-release", packageFiles("1.2.3", "0.9.1"), "base");
  f.refs.set("tags/v0.9.1", "published-release");
  f.pulls.push(pr(2, {
    user: app, state: "closed", merged_at: "2026-09-19T10:00:00Z", merge_commit_sha: "obsolete-release",
    body: releaseBody({ version: "0.9.1", pullRequests: [1] }),
    head: { ...pr().head, ref: releaseBranch, sha: "obsolete-release" },
  }));
  f.dependency(3, { merged: true, base: "published-release", files: packageFiles("1.2.4", "0.9.1") });
  await f.run();
  const release = f.pulls.find((pull) => pull.user.login === app.login && pull.state === "open");
  assert.ok(release, "Current dependency updates must still produce a release PR");
  assert.equal(release.title, "Release 0.9.2");
  assert.deepEqual(planFromBody(release.body), { version: "0.9.2", pullRequests: [3] });
  assert.equal(f.refs.get("tags/v0.9.1"), "published-release");
});

test("a reachable release PR still rejects a tag pointing to another commit", async () => {
  const f = fixture();
  f.commit("released", packageFiles("1.2.3", "0.9.1"), "base");
  f.refs.set("heads/main", "released");
  f.refs.set("tags/v0.9.1", "base");
  f.pulls.push(pr(2, {
    user: app, state: "closed", merged_at: "2026-09-19T10:00:00Z", merge_commit_sha: "released",
    body: releaseBody({ version: "0.9.1", pullRequests: [1] }),
    head: { ...pr().head, ref: releaseBranch, sha: "released" },
  }));
  await assert.rejects(f.run(), /existing tags are never moved/);
  assert.deepEqual(f.writes, []);
  assert.equal(f.refs.get("tags/v0.9.1"), "base");
});
