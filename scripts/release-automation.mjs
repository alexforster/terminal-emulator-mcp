...
export const requiredChecks = [
  "test (ubuntu-latest, 24.16.0)", "test (ubuntu-latest, 26)",
  "test (macos-latest, 24.16.0)", "test (macos-latest, 26)",
];
...
export function nextRelease(files) {
  ...
  const readme = files["README.md"].replaceAll(pin, `${manifest.name}@${version}`).replaceAll(
    `/releases/download/v${manifest.version}/${manifest.name}-${manifest.version}.tgz`,
    `/releases/download/v${version}/${manifest.name}-${version}.tgz`,
  );
